...
local cases = {
    equal = function(self)
        return function(expectedThing : any)
            ...
            assert(
               result,
               string.format("Expected value to %sbe %s, got %s",
                   never and " never" or "",
    ...
    exist = function(self)
        return function()
            ...
            assert(
                result,
                string.format("Expected value to%s exist, got %s",
                    never and " never" or "",
                    tostring(value)
                )
            )
    ...
    throw = function(self)
        return function()
            ...
            assert(
                result,
                string.format("Expected value to %serror", never and " never" or "")
            )
        end
    end,
    ...
    near = function(self)
        return function(expectToBeNear : number)
            ...
            assert(
                result,
                string.format("Expected %s to %sbe near '%s'",
                    tostring(value),
                    never and " never" or "",
